...
@WebServlet(name = "PastAgApQueryServlet", value = "/pagapq")
public class PastAgApQueryServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("进入pagapq.get");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("进入pagapq.post");
        String teauid = java.net.URLDecoder.decode((String) request.getSession().getAttribute("user"),"UTF-8");
        String tuid = teauid; //将session中存的当前用户id取出后解码
        ...
        if(role.equals("主讲教师")) {
            ...
            if(mq == null) {
                map = ApprovalService.selectPastAgApByLTuid(tuid,pageNum);
            } else {
                map = ApprovalService.selectPastAgApByLTuidLike(tuid,pageNum,suidQuery,courseQuery,stateQuery);
            }
            ...
            //将总条数存入最后一个ap用于前台展示
            approvalList.add(new Approval("pagapqTotalNum",Long.toString(pageInfo.getTotal())));


        } else if (role.equals("主管教师")) {
            Map<String,Object> map;
            if(mq == null) {
                map = ApprovalService.selectPastAgApBySTuid(tuid,pageNum);
            } else {
                map = ApprovalService.selectPastAgApBySTuidLike(tuid,pageNum,suidQuery,courseQuery,stateQuery);
            }
